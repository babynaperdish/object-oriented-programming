...
int main()
{
	...
	// 
	// если адреса векторов совпадают - это одно и то же => нет нужды приравнивать
	...
	Vector c = a;//потому что вызывается конструктор копирования по умолчанию
	...
	//Выбор обусловлен меньшей погрешностью при счете чисел типа double + не надо вычислять угол и синус
